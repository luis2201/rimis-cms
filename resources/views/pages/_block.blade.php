...
@elseif($block->type === 'image_card')
    <section class="page-block-section"><div class="mx-auto max-w-4xl px-5"><article class="page-block-image-card">@if(!empty($data['image_id']) && $media->has($data['image_id']))<img src="{{ $media[$data['image_id']]->publicUrl() }}" alt="{{ $media[$data['image_id']]->alt_text ?: ($data['title'] ?? '') }}">@endif<div class="page-block-image-card-body">@if(!empty($data['title']))<h2>{{ $data['title'] }}</h2>@endif<div class="page-public-content">{!! $data['content'] ?? '' !!}</div></div></article></div></section>
