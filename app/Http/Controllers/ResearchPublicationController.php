...
class ResearchPublicationController extends Controller
{
 ...
 private function saveAuthors(ResearchPublication $p,?array $authors,Request $r):void{$authors=collect($authors?:[['author_name'=>$r->user()->name,'institution'=>$r->user()->researcherProfile?->institution,'is_corresponding'=>true]])->filter(fn($a)=>filled($a['author_name']??null))->values();if($authors->isEmpty())return;$p->authors()->delete();$correspondingUsed=false;foreach($authors as $i=>$a){$isCorresponding=!$correspondingUsed&&!empty($a['is_corresponding']);$correspondingUsed=$correspondingUsed||$isCorresponding;$p->authors()->create(['user_id'=>$i===0?$r->user()->id:null,'author_name'=>$a['author_name'],'institution'=>$a['institution']??null,'orcid'=>$a['orcid']??null,'email'=>$a['email']??null,'author_order'=>$i+1,'is_corresponding'=>$isCorresponding]);}}
