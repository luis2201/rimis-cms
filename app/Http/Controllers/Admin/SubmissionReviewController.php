...
class SubmissionReviewController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        foreach($types as $type=>$class){
            $rows=$class::with(['author.researcherProfile','reviewer'])->where('origin',$class::ORIGIN_RESEARCHER)
                ...
                ->when($request->filled('institution'),fn($q)=>$q->whereHas('author.researcherProfile',fn($p)=>$p->where('institution','like','%'.$request->string('institution').'%')))
    ...
    private function authorizeEditor(Request $r,string $permission): void { abort_unless($r->user()->can($permission) && !$r->user()->hasRole('INVESTIGADOR'),403); }
