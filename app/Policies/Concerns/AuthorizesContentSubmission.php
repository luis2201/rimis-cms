...
trait AuthorizesContentSubmission
{
    ...
    public function createSubmission(User $user): bool { return $user->isMember() && $user->can('submissions.view-own'); }
    ...
    public function viewSubmission(User $user, object $model): bool { return ! $user->isMember() && $user->can('submissions.view') && $model->isResearcherSubmission() && ! $model->isReviewDraft(); }
    ...
    private function owns(User $user, object $model): bool { return $user->isMember() && $model->isResearcherSubmission() && (int) $model->user_id === (int) $user->id; }
}
